...
3. **Plan recovery** — change the autonomous objective (for example, stop cycling and park). On a validated intent tree, that terminal is an explicit `safeTerminal` mark in last-child fallback/recovery position, not a park-like action name.
